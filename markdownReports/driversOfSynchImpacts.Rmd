...
dum <- prodFullDat %>% 
  ...
  summarize(medRec = median(value),
            meanRec = mean(value))
ggplot(dum, aes(x = synch, y = meanRec, colour = model)) +
...
ggplot(aggDat, aes(x = synch, y = aggRecBY)) +
  labs(x = "Time", y = "Unsummarized Aggregate Recruitment") +
  geom_boxplot() +
  theme_sleekX()


# Caculate median over sim period (already filtered in srFullDat)
summAggDat <- aggDat %>% 
  gather(key = pm, value = value, aggRecBY, aggS) %>% 
  group_by(trial, synch, pm) %>% 
  summarize(meanPM = mean(value),
            medPM = median(value)) %>% 
  gather(key = summStat, value = value, meanPM, medPM)
