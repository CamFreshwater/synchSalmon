...
#     # }
#     synchList <- calcSynchMetrics(trimDatList, corr = FALSE, weight = TRUE,
#                                   windowSize = 12)
...
newAgTSList <- readRDS(here("outputs/generatedData/synchTS",
                          "retroForward_synchTSList.rda"))

# First make dataset that includes observed data collected within the simulation
...
# Next make identical dataframe but replacing observed data with outputs from TMB
...
First look at observed CVc and phi for short (all CUs within simulation) and long periods (equivalent to retro analysis) relative to sim values using medians among trials (i.e. interannual variability).
...
dum <- plotDat %>%
  filter(!synchOM %in% c("lowSynch", "highSynch"),
...
q1 <- ggplot(dum, aes(x = sigmaOM, y = medCompCVRecBY, fill = sigmaOM)) +
...
dum2 <- plotDat %>%
  filter(!sigmaOM %in% c("lowSigma", "highSigma"),
         !is.na(medSynchRecBY))
colPal2 <- c("black", "grey", viridis(length(unique(dum$sigmaOM)) - 2, 
                                      begin = 0, end = 1))
names(colPal2) <- levels(dum2$synchOM)
p1 <- ggplot(dum2, aes(x = synchOM, y = medSynchRecBY, fill = synchOM)) +
...
ggarrange(q1, p1, nrow = 2, ncol = 1, heights = c(1, 1.2))
```

As expected, longer TS result in greater interannual variability and lower medians (i.e. fewer years during high aggregate variability phase). While it's reasonable that observed values encompass plotted, I'm concerned that the medians draw criticism because they're so much lower than expected. I think that they're defensible though because we're including so many more populations in the forward simulation (18 vs. 10).
...
p2 <- ggplot(dum2, aes(x = synchOM, y = medSynchRecBY, fill = synchOM)) +
  ...
  guides(fill = FALSE, color = FALSE) +
  scale_fill_manual(values = colPal2) +
  scale_x_discrete(breaks = waiver(), labels = c("Obs E", "Obs L", 
...
This looks even messier and is sort of the worst of both worlds. It shows a subset of CUs (same apples and oranges argument as above), plus the decision of where to split into early and late is arbitrary. For now stick with simulated data from short time series but with all CUs included.