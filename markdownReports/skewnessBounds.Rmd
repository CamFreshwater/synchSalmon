...
for (i in 1:nStks) {
  ...
  p <- ggplot(dum, aes(x = skew, y = rec)) +
    ...
    facet_wrap(~covMat, scales = "free_y")
  print(p)
}
```
...
Even though we cannot explicitly quantify what proportion of the decline in productivity is being introduced by changes in skewness, as opposed to variance or covariance, I still believe there's value in contrasting what the two operating models suggest. Highly correlated negative recruitment deviations seem to be more ecologically relevant than uniform declines in the mean. My only concern is that we may have insufficient space to fully defend the approach.