...
First look at differences between different skewed covariate matrices.

``` {r plotDifferentSkewed}
#compare the two input covariance matrices
for (i in 1:nStks) {
  dum <- outDat %>% 
...
Overall outcomes are quite similar except for Harrison. This isn't entirely unexpected given that there is strong coherence within the aggregate, except for this sea-type stock. 

Next compare the multivariate normal distributions to the skewed with custom covariance matrix (default results were qualitatively similar).
``` {r compareSkewedMVN}
...
Varies among CUs, but generally low alpha produces recruit abundances smaller than even relatively severe skewness parameters. 
...
Depending on CUs even high skewness and high covariance doesn't result in realized productivity patterns that are more severe than Kalman filter estimates.

All that being said I don't think we can get around the issue that greater covariance drives single CU abundance down. I just don't think that we can identify an ecologically plausible mechanism for that.
...
for (i in seq_along(dirNames)) { #make dataframe
  ...
  stkNames <- cuList[["stkName"]]
  recBYMat <- cuList[["medRecBY"]]
  colnames(recBYMat) <- stkNames
  nTrials <- nrow(recBYMat)
  recBYDat <- data.frame(om = rep(omNames[i], 
                                  length.out = nTrials * length(stkNames)))
  dum <- recBYMat %>%
    as.data.frame() %>%
    gather(key = cu, value = rec)
  plotDat <- rbind(plotDat, cbind(recBYDat, dum))
}
...
for (i in seq_along(omNames)) { #make dataframe
  ...
  plotList <- lapply(cuList, function(h) {
    ...
    stkNames <- h[["stkName"]]
    colnames(recMat) <- stkNames
    nTrials <- nrow(recMat)
    recBYDat <- data.frame(om = rep(omNames[i], length.out = nTrials * nCUs),
