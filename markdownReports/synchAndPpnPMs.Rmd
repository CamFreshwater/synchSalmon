...
outList <- lapply(seq_along(rSeq), function(x) {
  set.seed(456)
  dum <- ricSim(N = 10000, r = rSeq[x], sigmas = sigma * sigScalar[y]) %>% 
...
outList <- lapply(seq_along(dirNames), function(i) {
  ...
  dum <- lapply(agList, function(x) {
    x %>% 
      select(opMod, trial, ppnCUUpper, ppnCULower, ppnYrsAllMixOpen,
             ppnMixedOpen) %>% 
...
outDat <- do.call(rbind, outList) %>% 
  gather(key = pm, value = value, ppnCUUpper, ppnCULower, ppnYrsAllMixOpen,
         ppnMixedOpen) %>% 
  mutate(synch = factor(factor(synch), levels = c("lowSynch", "medSynch",
                                                   "highSynch")))

plotDat <- outDat %>% 
  filter(pm == "ppnMixedOpen")
...
**Yes** appear to be driven by exploitation. I.e. when populations are highly synchronized, HCRs are more effective at limiting overharvest (slightly).
...
outterList <- lapply(seq_along(sigScalar), function(y) {
  outList <- lapply(seq_along(rSeq), function(x) {
    set.seed(456)
    dum <- ricSim(N = 1000, r = rSeq[x], sigmas = sigma * sigScalar[y]) %>% 
...
ggplot(dum2, aes(x=sigScalar, y=rec, fill=r)) + 
  geom_boxplot() +
  facet_wrap(~CU, scales = "free_y")
