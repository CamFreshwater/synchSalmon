...
agList <- genOutputList(dirNames[2], agg = TRUE, aggTS = TRUE)
...
First compare estimates of median ppn through time to means (default). Since absolute abundance PMs are based on medians need to confirm that using means doesn't bias results for ppn PMs.

``` {r medPpnVsMeanPpn}
ppnUpList <- lapply(seq_along(dirNames), function (x) {
  agList <- genOutputList(dirNames[x], agg = TRUE, aggTS = TRUE)
  makeData(agList, "Prop Above Upper BM") %>% 
    dplyr::rename(ppnUp = varName) %>% 
    filter(!is.na(ppnUp)) %>%
    dplyr::mutate(synch = factor(synch, levels = c("lowSynch", "medSynch",
                                                 "highSynch")),
                  sigma = factor(scenNames[x]))  
})
fullPpnUpDat <- do.call(rbind, ppnUpList)

meanDat <- fullPpnUpDat %>%
  dplyr::group_by(trial, synch, sigma) %>% 
  dplyr::summarize(meanUp = mean(ppnUp),
                   medUp = median(ppnUp)) %>% 
  gather(key = stat, value = ppnUp, meanUp, medUp)

ggplot(meanDat, aes(x = synch, y = ppnUp, colour = stat)) +
  geom_boxplot() +
  theme_sleekX() +
  facet_wrap(~sigma)
...
Approximately similar patterns, keep mean for now to avoid rerunning sims unless co-authors object.
...
meanDat %>% 
  group_by(trial, synch) %>% 
  summarize(meanUp = median(meanUp),
            medUp = median(medUp))
```
...
simPar <- read.csv(here("data/sox/fraserOMInputs_varyCorr.csv"),
                   stringsAsFactors = F)
simParTrim <- simPar %>% 
  filter(scenario %in% c("medSig", "medSigFixed"))
scenNames <- unique(simParTrim$scenario)
dirNames <- sapply(scenNames, function(x) paste(x, unique(simParTrim$species), 
                                                sep = "_"))
...
outList <- lapply(seq_along(dirNames), function(i) {
  ...
  dum <- lapply(agList, function(x) {
    x %>% 
      select(opMod, manProc, trial, ppnCUUpper, ppnCULower, ppnMixedOpen) %>% 
      dplyr::rename(synch = opMod)
  })
  do.call(rbind, dum)
})
...
Also confirmed looking at TAM vs. fixed exploitation rate.

Do these drivers also explain trends in single CU PMs, specifically the increase in spawner abundance observed with greater synchrony?
```{r noMortSimsSAbundance}
simPar <- read.csv(here("data/sox/fraserOMInputs_varyCorrNoMort.csv"),
                   stringsAsFactors = F)
simParTrim <- simPar %>% 
  filter(scenario %in% c("ref", "noExp", "noMM", "noBoth"))
...
for (i in seq_along(dirNames)) { #make dataframe
  ...
  plotList <- lapply(cuList, function(h) {
    ...
    spwnDat <- data.frame(om = rep(scenNames[i], length.out = nTrials * nCUs),
...
plotDat <- do.call(rbind, fullPlotList) %>%
  mutate(synch = factor(factor(synch),
                        levels = c("lowSynch", "medSynch", "highSynch"))) %>% 
  filter(cu == "E.St")
row.names(plotDat) <- NULL

ggplot(plotDat, aes(x = synch, y = spawners)) +
  geom_boxplot() +
  theme_sleekX() +
  facet_wrap(~om)

plotDat %>% 
  # filter(om %in% c("ref", "noExp")) %>% 
  dplyr::group_by(om, synch) %>% 
  dplyr::summarize(median(spawners))

```

Again, trends in spawner abundance associated with increasing synchrony disappear when exploitation removed.
