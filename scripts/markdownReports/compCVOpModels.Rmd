...
Carrie used a simple simulation to determine that recruitment declines at high levels of component variability (i.e. sigma), but only when density dependence was included; otherwise only variability increases. Sean hypothesized that this might be due to a greater proportion of recruitment events occurring at the far end of the stock-recruitment curve when deviations are high, resulting in spawner abundance swinging between extreme. Plot stock recruitment curves to visualize. Note that all of these results are from reference (i.e. non-skewed) productivity OMs.

```{r importData, warning=FALSE, include=FALSE}
...
```{r cleanData, warning=FALSE, include=FALSE}
...
```{r plotSRCurves, echo=TRUE, warning=FALSE}
...
plotList <- lapply(seq_along(drawTrials), function(i) { 
  ...
  r <- ggplot(dum, aes(x = spwn, y = rec, colour = sig)) +
...
Results (only Chilko shown here) show the increase in scatter along the SR curve as sigma goes up (as expected). Large declines could be driven by synergistic negative impacts of density dependence and intermittent strong negative recruitment deviations, i.e. overcompensation may occur due to chance alone. In theory these may be modified by autocorrelation and the size of beta. To explore first run a series of scenarios across different levels of sigma with reference settings, reduced beta, or no autocorrelation.
...
consPlots <- lapply(seq_along(consVars), function(i) {
  ...
  q <- ggplot(temp, aes(x = sigma, y = avg, ymin = lowQ, ymax = highQ, 
    ...
    labs(x = "Component Variance", y = consYLabs[i], 
         color = "Sim.\nParameter\nValue") +
    geom_pointrange(fatten = 3.5, size = 0.8, 
...
Unsurprisingly decreasing beta increases recruit abundance and decreases BM status. Removing AR has no impact on median recruit abundance, but increases the proportion of CUs above upper BM and reduces the likelihood of CUs going extinct to nil. Effect of component variance and synchrony appears to be similar (i.e. purple symbols still show an upward trend in recruit abundance). 
...
Low beta appears to reduce impact of greater variability, but only for Bowron. Note that trend appears to be CU-specific (CU-specific differences are examined further in the cuSpecificPMs.Rmd). This suggests that there are some funny interactions going on between SR parameters. While these two stocks have fairly similar alpha parameters, Chilko's beta is tiny. 

Finally look at SR curves of different operating models.
...
plotList <- lapply(seq_along(omNames), function(i) { 
  dum <- srFull %>% 
    #focus on Chilko data only
  ...
  r <- ggplot(dum, aes(x = spwn, y = rec, colour = sig)) +
    ...
    geom_point(size = 1.25) +
    scale_colour_discrete(name = "Sigma") +
    theme_sleekX(position = "bottom") +
    theme(axis.title.y=element_blank()) +
    facet_wrap(~sig, nrow = length(unique(srFull$sig)), ncol = 1)
  return(r)
...
Looking at individual trials, decreasing beta does appear to reduce the impacts of greater variability, while removing temporal autocorrelation does not. Presumably strong compensation causes big swings back and forth in recruit abundance and enough of these are sufficiently large and negative to drive median recruit abundance down. However it's still not clear to me _why_ this results in a decline rather than just greater variability.