...
In compCVOpModels.Rmd I noticed that patterns in CU-specific PMs varied among CUs. Focusing on reference op model explore how CUs differ in effects of greater synchrony (under median sigma) and greater CVc (under median synch). Note that while the interaction between synch and CVc will also vary among CUs, it is going to be difficult to fully explore this and incorporate into the synchrony manuscript. Therefore the goal here is simply to get an understanding of variability among CUs.
...
q
```
The majority of CUs decline with increasing variability, but the strength of this effect varies. Notable exceptions are Gates/Nadina (medians stable even though variance increases) and Cultus (median increases along with variance).

How are these related to beta values?

```{r plotBeta, echo = FALSE}
betaDat <- data.frame(cu <- cuList$stkName,
                      medBeta <- cuList$medBeta[1,])
plot(medBeta ~ as.factor(cu), data = betaDat)
```