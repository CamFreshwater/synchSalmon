...
```{r runSimulations, warning=FALSE, include=FALSE}
...
```{r prepOutputData, warning=FALSE, include=FALSE}
...
annualMeans <- dumFull %>% 
  ...
  #use means AMONG years (i.e. violin plots show intertrial variability)
  summarise(meanLogRS = mean(logRS))  
## means among trials
trialMeans <- dumFull %>% 
  group_by(cu, yr, om) %>%
...
Some variability among CUs, but on the whole Kalman smoothed a and skewed normal have similar realized productivity. 